...
class ProviderController extends Controller {
    ...
    public function getShow(Request $request) {
        $datas  = Provider::where('parent_key', '=', 0)->get()->toArray();
        $result = [];
        foreach ($datas as $k => $item) {
            $_data    = Provider::where('parent_key','=',$item['id'])->get()->toArray();
            $chilData = [];
    ...
    public function getListbyid(Request $request) {
        $id      = $request->input('id');
        $getData = Provider::find($id);
        return $this->ResponseData($getData);
    }

    public function postAdd(Request $request){
        $data       = [];
        $col_data   = DB::table('provider')->lists('name');
        $id         = $request->input('id');
    ...
    private function _add($name, $parent_key, $ordinal, $active, $col_data) {
        try {
            if(in_array($name,$col_data)) {
                ...
                $Provider->active      = $active ? $active : 2; //1 is active and 2 is non active Provider
                $rs                    = $Provider->save();
    ...
    public function postUpdate(Request $request) {
        ...
        foreach ($dataRq as $k => $data) {
            ...
            DB::table('provider')->where('id', $id_parent)->update(['parent_key' => 0]);

            if(isset($data['children'])) {
                foreach ($data['children'] as $item) {
                    $child_id =  $item['item']['id'];
                    $rs       = DB::table('provider')->where('id', $child_id)->update(['parent_key' => $id_parent]);
                }
            }
    ...
    public function postDelbyid(Request $request) {
        ...
        try {
            ...
            $obj->delete();
            $this->message = 'Xóa nhà cung cấp thành công !';
        } catch (Exception $e) {
            $this->error         = false;
            $this->error_message = $e->getMessage();
        }
